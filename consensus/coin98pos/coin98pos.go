...
// Coin98PosRLP returns the rlp bytes which needs to be signed for the Coin98Pos
...
func Coin98PosRLP(header *types.Header, chainId *big.Int) []byte {
	b := new(bytes.Buffer)
	encodeSigHeader(b, header, chainId)
	return b.Bytes()
}

// Coin98Pos is the consensus engine of BSC
type Coin98Pos struct {
	chainConfig *params.ChainConfig  // Chain config
	config      *params.Coin98PosConfig // Consensus engine configuration parameters for Coin98Pos consensus
...
// New creates a Coin98Pos consensus engine.
func New(
	...
	// get Coin98Pos config
	Coin98PosConfig := chainConfig.Coin98Pos

	// Set any missing consensus parameters to their defaults
	if Coin98PosConfig != nil && Coin98PosConfig.Epoch == 0 {
		Coin98PosConfig.Epoch = defaultEpochLength
	}
...
func (p *Coin98Pos) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	// deploy a contract
	if tx.To() == nil {
		return false, nil
	}
...
func (p *Coin98Pos) IsSystemContract(to *common.Address) bool {
	if to == nil {
		return false
	}
...
func (p *Coin98Pos) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (p *Coin98Pos) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (p *Coin98Pos) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (p *Coin98Pos) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Coin98Pos) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Coin98Pos) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (p *Coin98Pos) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (p *Coin98Pos) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
...
// from.
func (p *Coin98Pos) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Coin98Pos) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// rewards given.
func (p *Coin98Pos) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction,
	...
	cx := chainContext{Chain: chain, Coin98Pos: p}
...
func (p *Coin98Pos) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
...
// with.
func (p *Coin98Pos) Authorize(val common.Address, signFn SignerFn, signTxFn SignerTxFn) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *Coin98Pos) Delay(chain consensus.ChainReader, header *types.Header) *time.Duration {
	number := header.Number.Uint64()
	snap, err := p.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
		return nil
...
func (p *Coin98Pos) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	// Sign all the things!
	sig, err := signFn(accounts.Account{Address: val}, accounts.MimetypeCoin98Pos, Coin98PosRLP(header, p.chainConfig.ChainID))
	if err != nil {
		return err
	}
...
func (p *Coin98Pos) shouldWaitForCurrentBlockProcess(chain consensus.ChainHeaderReader, header *types.Header, snap *Snapshot) bool {
	if header.Difficulty.Cmp(diffInTurn) == 0 {
		return false
	}
...
func (p *Coin98Pos) EnoughDistance(chain consensus.ChainReader, header *types.Header) bool {
...
func (p *Coin98Pos) AllowLightProcess(chain consensus.ChainReader, currentHeader *types.Header) bool {
...
func (p *Coin98Pos) IsLocalBlock(header *types.Header) bool {
	return p.val == header.Coinbase
}

func (p *Coin98Pos) SignRecently(chain consensus.ChainReader, parent *types.Header) (bool, error) {
...
// current signer.
func (p *Coin98Pos) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (p *Coin98Pos) SealHash(header *types.Header) common.Hash {
...
func (p *Coin98Pos) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "Coin98Pos",
		Version:   "1.0",
		Service:   &API{chain: chain, Coin98Pos: p},
		Public:    false,
	}}
}

// Close implements consensus.Engine. It's a noop for Coin98Pos as there are no background threads.
func (p *Coin98Pos) Close() error {
	return nil
}
...
func (p *Coin98Pos) getCurrentValidators(blockHash common.Hash) ([]common.Address, error) {
	// block
...
func (p *Coin98Pos) distributeIncoming(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
...
// slash spoiled validators
func (p *Coin98Pos) slash(spoiledVal common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	method := "slash"
...
// init contract
func (p *Coin98Pos) initContract(state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	method := "init"
	// contracts
...
func (p *Coin98Pos) distributeToSystem(amount *big.Int, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// get system message
...
// slash spoiled validators
func (p *Coin98Pos) distributeToValidator(amount *big.Int, validator common.Address,
...
// get system message
func (p *Coin98Pos) getSystemMessage(from, toAddress common.Address, data []byte, value *big.Int) callmsg {
	return callmsg{
		ethereum.CallMsg{
			From:     from,
...
func (p *Coin98Pos) applyTransaction(
	msg callmsg,
	state *state.StateDB,
	header *types.Header,
